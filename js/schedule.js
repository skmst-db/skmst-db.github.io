...
let cachedBiographyRows = null;

async function fetchBiographyRows() {
    if (cachedBiographyRows) return cachedBiographyRows;
    try {
        ...
        cachedBiographyRows = rows.slice(1).map(row => {
        ...
        return cachedBiographyRows;
    } catch (error) {
        console.error('Error loading biography CSV rows:', error);
        return [];
    }
}
...
async function fetchBiographyCSV() {
    ...
    try {
        const biographyRows = await fetchBiographyRows();
        cachedBiographyEvents = biographyRows.map(eventData => {
        ...
        }).filter(event => event && (event.title || event.name));
