...
function initActivityMapView(showLeadRoleOnly = false, selectedTypes = [], showNonLeadOnly = false, showAwardOnly = false) {
    ...
    activityMapState.pendingInit = ensureActivityMapLibraries()
        .then(() => Promise.all([
            ...
            fetch('data/location.csv').then(response => response.text()).then(parseCSVText)
        ]))
...
function toScheduleEvent(item) {
    ...
    return {
        ...
        excludeDates: parseAndFormatDates(item.DateDelete),
        additionalDates: parseAndFormatDates(item.DateAdd)
    };
}
...
function getFilteredActivityDates(item) {
    ...
    let currentIterateDate = new Date(event.startDate);
    currentIterateDate.setUTCHours(event.startDate.getUTCHours(), event.startDate.getUTCMinutes(), 0, 0);
    const endDate = new Date(event.endDate || event.startDate);
    endDate.setUTCHours((event.endDate || event.startDate).getUTCHours(), (event.endDate || event.startDate).getUTCMinutes(), 0, 0);
    let safetyCounter = 0;
    while (currentIterateDate <= endDate && safetyCounter < 36600) {
        if (isEventActiveOnDate(event, currentIterateDate)) {
    ...
    (event.additionalDates || []).forEach(dateStr => {
        if (dateStr) relevantDates.add(dateStr);
    });
...
function showWorksDetail(key, worksDataMap, viewType) {
    ...
    const detailHeaderTitle = document.getElementById('detail-title');
    ...
    if (detailHeaderTitle) detailHeaderTitle.textContent = title;

    if (works.length === 0) {
        detailContent.innerHTML = `<p>No works found.</p>`;
    } else {
        const worksByType = {};
        works.forEach(work => {
            const nType = normalizeWorksType(work.WorksType);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Promise.all([
        ...
        worksData = parseCSVText(bioData);
        locationData = parseCSVText(locData);
