document.addEventListener('DOMContentLoaded', async function () {
    ...
    function hasUwasaFlag(noteText) {
        return /(^|[\s,;|])uwasa($|[\s,;|])/i.test(noteText || '');
    }

    function hasMemoFlag(noteText) {
        return /(^|[\s,;|])memo($|[\s,;|])/i.test(noteText || '');
    }

    function extractYearFromNote(noteText) {
        if (!noteText) return 0;
        const normalized = noteText.replace(/[０-９]/g, (c) => String.fromCharCode(c.charCodeAt(0) - 0xFEE0));
        const matches = normalized.match(/(?:19|20)\d{2}/g);
    ...
    async function loadData() {
        try {
            const rows = await fetchBiographyRows();

            allWorks = rows
                .map(row => {
                    const note = row.Note || '';
                    if (hasUwasaFlag(note) || hasMemoFlag(note)) {
                        return null;
                    }
                    const dateStart = parseJSTDate((row.DateStart || '').trim());
                    ...
                    if (dateStart) {
                        ...
                        if (noteText && !hasUwasaFlag(noteText)) {
                            year = extractYearFromNote(noteText);
                        }
                    }
