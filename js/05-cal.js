 ...
 * parseSyukujitsuCSV is provided by schedule.js
 */
    ...
    months.forEach((monthName, monthIndex) => {
        ...
        for (let day = 1; day <= lastDay.getUTCDate(); day++) {
            ...
            if (dayEvents.length > 0) {
                ...
                } else if (sources.has('special_memo')) {
...
function showUpcomingDetails(date, events) {
    ...
    const nonHolidayEvents = events.filter(e => e.source !== 'syukujitsu');
    ...
    const eventsWithDates = nonHolidayEvents.map(e => {
        ...
        if (!originalDate) {
            ...
            else if (e.source === 'special_memo' || e.source === 'syukujitsu') originalDate = new Date(0);
...
function renderEvents(calendarGrid, year, month, events) {
    ...
    events.forEach(event => {
        ...
        else if (event.source === 'special_memo') backgroundColor = '#43AA8B';
        else if (event.source === 'other') backgroundColor = 'rgba(241, 196, 15, 0.8)';
        else if (event.source === 'anniversary') backgroundColor = event.color || '#889063';
...
/**
 * Shared Helpers (Movie Window Period)
 */

// [Used by: Upcoming, Week]
...
/**
 * Shared with Year & Upcoming (Event Status)
 */

// [Used by: Year, Upcoming] (Movie window ONLY applies to Upcoming)
function getEventStatusOnDate(event, date) {
    let isActive = false;
    let isMovieShowing = false;

    if (event.worksType === '映画' && currentViewMode === 'upcoming') {
...
/**
 * Shared with Schedule & Anniversary (Milestones & Formatting)
 */
    ...
    for (let d = 1; d <= lastDayOfMonth; d++) {
        ...
        if (dayCount > 0 && dayCount % 100 === 0) {
            milestones.push({
                ...
                title: `${dayCount}th`, url: 'https://sakai-masato.com/',
...
function formatAnniversaryEvent(event) {
    ...
    let color = (event.role === '主演') ? '#5474a1' : '#889063';
    ...
    if (event.anniversaryType === 'start') title += ` 【${event.diffValue}周年】`;
        ...
        title += ` 【${event.diffValue}日前】`;
    } else if (event.anniversaryType === 'days_end') {
        title += ` 【終了-${event.diffValue}日前】`;
    }
